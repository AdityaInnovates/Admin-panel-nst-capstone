...
function Stable() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await axios.get("https://a209-115-244-141-202.ngrok-free.app/studentlist", {
                    headers: {
                    ...
                    {Array.isArray(tableData) && tableData.map(item => (
                        ...
                        <td className="px-2 py-2">
                            <div className='flex items-center justify-center'>
                                <a href={item.githubRepo} target='_blank' rel="noopener noreferrer"><img src={link} alt="Github link" className='h-5'/></a>
                            </div>
                        </td>
             
                        <td className="px-2 py-2">
                            <div className='flex items-center justify-center'>
                                <a href={item.hostedList} target='blank'><img src={link} className='h-5'/></a>
                            </div>
                        </td>
                        <td className="px-2 py-2">
                        ...
                        <td className="px-2 py-2 ">
                            <div className='flex justify-center items-center'>{item.report.feedback}</div>
                        </td>
                        <td className="px-2 py-2">
                            <div className='flex justify-center items-center'>
                            <button disabled={item.report.completionStatus === "Done"} 
                                    onClick={() => navigate('/evaluate')}>Go</button>

                            </div>
                        </td>
                        <td className="px-2 py-2">
                            <div className='flex items-center justify-center'><button aria-label={`Send email to ${item.email}`}>Send</button></div>
                        </td>
                        </tr>
                    ))}
                    </tbody>
